        ...
        try {
            try {
                const [t, c, e] = await Promise.all([
                    fetchTurmas(),
                    fetchCursos(),
                    supabase.from('enrollments').select('id, aluno, turma, pacote').eq('situacao', 'ATIVO')
                ]);
                setData(t);
                setCursos(c);
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    <div className="relative">
                        ...
                        <input type="text" placeholder="Buscar turma..." value={search} onChange={e => setSearch(e.target.value)} className="pl-10 pr-4 py-3 bg-white dark:bg-zinc-900 transition-colors border border-zinc-200 dark:border-zinc-700 transition-colors rounded-xl text-sm font-semibold text-[#231F20] dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-[#E31E24] w-48" />
                    </div>
                    <select value={selectedCourse} onChange={e => setSelectedCourse(e.target.value)} className="px-4 py-3 bg-white dark:bg-zinc-900 transition-colors border border-zinc-200 dark:border-zinc-700 transition-colors rounded-xl text-sm font-semibold text-[#231F20] dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-[#E31E24] w-48">
            ...
            <div className="bg-white dark:bg-zinc-900 transition-colors rounded-[2rem] shadow-sm overflow-hidden">
                {loading ? (
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody>
                            ...
                            {filtered.map((t, i) => {
                                const students = getStudents(t.nome, t.curso_nome || '');
                                const isExpanded = expandedTurma === t.id;
                                return (
                                    <React.Fragment key={t.id}>
                                        <tr className={`border-t border-zinc-100 dark:border-zinc-800 transition-colors hover:bg-zinc-50 dark:bg-zinc-950/50 transition-colors cursor-pointer ${i % 2 === 0 ? 'bg-white dark:bg-zinc-900 transition-colors' : 'bg-zinc-50 dark:bg-zinc-950/50 /50 transition-colors'} ${isExpanded ? 'bg-zinc-100 dark:bg-zinc-800 transition-colors' : ''}`} onClick={() => setExpandedTurma(isExpanded ? null : t.id)}>
                                            <td className="px-6 py-4 font-black text-sm text-[#231F20] dark:text-zinc-100">
                                            ...
                                            <td className="px-6 py-4" onClick={e => e.stopPropagation()}>
                                                <div className="flex items-center justify-center gap-2">
                                                    <button onClick={() => openEdit(t)} className="p-2 bg-zinc-100 dark:bg-zinc-800 transition-colors rounded-lg hover:bg-[#E31E24] hover:text-white transition-all text-zinc-500" title="Editar">
                                                    ...
                                                    {deleteConfirm === t.id ? (
                                                        ...
                                                        <button onClick={() => setDeleteConfirm(t.id)} className="p-2 bg-zinc-100 dark:bg-zinc-800 transition-colors rounded-lg hover:bg-red-600 hover:text-white transition-all text-zinc-500" title="Excluir">
                                        ...
                                        {isExpanded && (
                                            <tr className="bg-zinc-50 dark:bg-zinc-950/50 /50 transition-colors">
                                                <td colSpan={6} className="px-6 py-4 border-t border-zinc-100 dark:border-zinc-800 transition-colors shadow-inner">
                ...
                <div className="px-6 py-4 bg-zinc-50 dark:bg-zinc-950/50 transition-colors border-t border-zinc-100 dark:border-zinc-800 transition-colors"><p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">{filtered.length} {filtered.length === 1 ? 'turma' : 'turmas'}</p></div>
            </div>
