...
const AIAssistant: React.FC<Props> = ({ isOpen, onClose, stats }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex justify-end">
            ...
            <div className="relative w-full md:w-[400px] bg-white dark:bg-zinc-900 transition-colors h-full shadow-2xl flex flex-col">
                {/* Header */}
                ...
                <div className="flex-grow overflow-y-auto p-4 space-y-4 bg-[#F8F9FA] custom-scrollbar">
                    {messages.map((msg, idx) => (
                        <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[85%] p-4 rounded-2xl shadow-sm text-sm leading-relaxed ${msg.role === 'user'
                                ...
                                : 'bg-white dark:bg-zinc-900 text-zinc-700 rounded-tl-none border border-zinc-100 dark:border-zinc-800 transition-colors'
                                }`}>
                    ...
                    {loading && (
                        <div className="flex justify-start">
                            <div className="bg-white dark:bg-zinc-900 transition-colors p-4 rounded-2xl rounded-tl-none border border-zinc-100 dark:border-zinc-800 transition-colors shadow-sm">
                ...
                {/* Input */}
                <div className="p-4 bg-white dark:bg-zinc-900 transition-colors border-t border-zinc-100 dark:border-zinc-800 transition-colors">
                    <div className="flex gap-2">
                        <input
                            ...
                            disabled={loading}
                            className="flex-grow px-4 py-3 bg-zinc-50 dark:bg-zinc-950/50 transition-colors border border-zinc-200 dark:border-zinc-700 transition-colors rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#E31E24] disabled:opacity-50"
                        />
                        <button
                            onClick={handleSend}
