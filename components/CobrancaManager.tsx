...
const CobrancaManager: React.FC = () => {
                ...
                {loading ? (
                    ...
                    <table className="w-full text-left">
                        <thead><tr className="bg-zinc-50 border-b border-zinc-200">
                            ...
                            <th className="px-6 py-3 text-[9px] font-black text-zinc-500 uppercase tracking-widest text-right">A Receber</th>
                        </tr></thead>
                        <tbody>
                            {filteredPendentes.length === 0 && <tr><td colSpan={5} className="px-6 py-16 text-center text-zinc-400 font-bold uppercase tracking-widest text-[10px]">Nenhum aluno com assinatura pendente {selectedVendor !== 'todos' ? 'para este vendedor' : ''} 🎉</td></tr>}
                            {filteredPendentes.map((p, i) => (
                                <tr key={p.id} className={`border-t border-zinc-100 hover:bg-zinc-50 ${i % 2 ? 'bg-zinc-50/50' : ''}`}>
                                    <td className="px-6 py-4 font-black text-sm text-[#231F20]">{p.aluno || '—'}</td>
                                    <td className="px-6 py-4 text-xs font-bold text-zinc-600">{p.pacote || '—'}</td>
                                    <td className="px-6 py-4"><span className="inline-block bg-[#231F20] text-white text-[9px] font-black px-2.5 py-1 rounded-full">{p.turma || '—'}</span></td>
                                    <td className="px-6 py-4 text-xs font-bold text-zinc-600">{p.atendente || '—'}</td>
                                    <td className="px-6 py-4 text-right text-xs font-black text-[#E31E24]">R$ {(p.total_a_receber || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
