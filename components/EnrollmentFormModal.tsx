...
const EnrollmentFormModal: React.FC<Props> = ({ enrollment, onSave, onClose, saving }) => {
    ...
    const inputCls = "w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-950/50 transition-colors border border-zinc-200 dark:border-zinc-700 transition-colors rounded-xl text-sm font-semibold text-[#231F20] dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-[#E31E24] transition-all";
    const labelCls = "block text-[9px] font-black text-zinc-500 uppercase tracking-widest mb-1.5";

    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white dark:bg-zinc-900 transition-colors rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="p-8">
                    ...
                    <div className="flex justify-end gap-3 mt-8 pt-6 border-t border-zinc-100 dark:border-zinc-800 transition-colors">
                        <button type="button" onClick={onClose} disabled={saving} className="px-6 py-3 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 font-black uppercase tracking-widest text-[10px] rounded-xl hover:bg-zinc-200 transition-all">Cancelar</button>
