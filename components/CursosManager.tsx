    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6">
            ...
            <div className="bg-white dark:bg-zinc-900 transition-colors rounded-[2rem] shadow-sm overflow-hidden">
                                ...
                                {filtered.map((c, i) => {
                                    ...
                                    return (
                                        <React.Fragment key={c.id}>
                                            <tr className={`border-t border-zinc-100 dark:border-zinc-800 transition-colors hover:bg-zinc-50 dark:bg-zinc-950/50 transition-colors ${i % 2 === 0 ? 'bg-white dark:bg-zinc-900 transition-colors' : 'bg-zinc-50 dark:bg-zinc-950/50 /50 transition-colors'}`}>
                                                <td className="px-6 py-4 font-black text-sm text-[#231F20] dark:text-zinc-100">{c.nome}</td>
                                                <td className="px-6 py-4 text-xs text-zinc-600">{c.descricao || '—'}</td>
                                                <td className="px-6 py-4 text-right text-xs font-black text-[#231F20] dark:text-zinc-100">R$ {c.valor.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <button
                                                        ...
                                                        className={`inline-flex items-center gap-1.5 text-[10px] font-black px-3 py-1.5 rounded-full transition-all ${isExpanded ? 'bg-[#E31E24] text-white shadow-md' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-600 hover:bg-[#E31E24] hover:text-white'}`}
                                                        title="Ver alunos"
                                                    >
                                            ...
                                            {isExpanded && (
                                                <tr>
                                                    <td colSpan={6} className="bg-zinc-50 dark:bg-zinc-950/50 transition-colors px-6 py-4 border-t border-zinc-200 dark:border-zinc-700 transition-colors">
                                                        {/* Payment Filter Tabs */}
                                                        <div className="flex flex-wrap items-center gap-2 mb-4">
                                                            <span className="text-[9px] font-black text-zinc-500 uppercase tracking-widest mr-2">Filtrar:</span>
                                                            <button onClick={() => setPaymentFilter('todos')} className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all ${paymentFilter === 'todos' ? 'bg-[#231F20] text-white shadow' : 'bg-white dark:bg-zinc-900 text-zinc-500 border border-zinc-200 dark:border-zinc-700 transition-colors hover:border-zinc-400'}`}>
                                                                Todos ({stats.total})
                                                            </button>
                                                            <button onClick={() => setPaymentFilter('pago')} className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all ${paymentFilter === 'pago' ? 'bg-green-600 text-white shadow' : 'bg-white dark:bg-zinc-900 text-zinc-500 border border-zinc-200 dark:border-zinc-700 transition-colors hover:border-green-400'}`}>
                                                                ✓ Pagos ({stats.paid})
                                                            </button>
                                                            <button onClick={() => setPaymentFilter('pendente')} className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all ${paymentFilter === 'pendente' ? 'bg-amber-500 text-white shadow' : 'bg-white dark:bg-zinc-900 text-zinc-500 border border-zinc-200 dark:border-zinc-700 transition-colors hover:border-amber-400'}`}>
                                                        ...
                                                        {students.length === 0 ? (
                                                            ...
                                                            <div className="overflow-x-auto rounded-xl border border-zinc-200 dark:border-zinc-700 transition-colors">
                                                                <table className="w-full text-left bg-white dark:bg-zinc-900 transition-colors">
                                                                    <thead><tr className="bg-zinc-100 dark:bg-zinc-800 transition-colors">
                                                                        ...
                                                                        <th className="px-4 py-2.5 text-[9px] font-black text-zinc-500 uppercase tracking-widest text-center">Contrato</th>
                                                                    ...
                                                                    <tbody>
                                                                        {students.map((s, idx) => {
                                                                            const isPaid = s['Total Recebido'] > 0;
                                                                            return (
                                                                                <tr key={s.id || idx} className={`border-t border-zinc-100 dark:border-zinc-800 transition-colors ${idx % 2 ? 'bg-zinc-50 dark:bg-zinc-950/50 transition-colors/30' : ''}`}>
                                                                                    <td className="px-4 py-3 text-xs font-bold text-[#231F20] dark:text-zinc-100">{s.Aluno}</td>
                                                                                    ...
                                                                                    <td className="px-4 py-3 text-center">
                                                                                        {s.Assinatura === 'DIGITAL' ? (
                                                                                            <span className="inline-block px-2.5 py-1 rounded-md text-[9px] font-black uppercase bg-blue-100 text-blue-700 border border-blue-200" title="Assinatura Digital">
                                                                                                💻 Digital
                                                                                            </span>
                                                                                        ) : s.Assinatura === 'PRESENCIAL' ? (
                                                                                            <span className="inline-block px-2.5 py-1 rounded-md text-[9px] font-black uppercase bg-zinc-100 dark:bg-zinc-800 text-zinc-600 border border-zinc-200 dark:border-zinc-700 transition-colors" title="Assinatura Presencial">
                                                                                                📝 Presen.
                                                                                            </span>
                                                                                        ) : (
                                                                                            <span className="inline-block px-2.5 py-1 rounded-md text-[9px] font-black uppercase bg-red-50 text-red-600 border border-red-200" title="Sem Contrato">
                                                                                                ⚠️ Nenhum
                                                                                            </span>
                                                                                        )}
                                                                                    </td>
                                                                                    <td className="px-4 py-3 text-center">
                                                                                        <button onClick={() => handleEditEnrollment(s)} className="p-1.5 bg-zinc-100 dark:bg-zinc-800 transition-colors rounded-lg hover:bg-[#E31E24] hover:text-white transition-all text-zinc-500" title="Editar Matrícula">
                ...
                <div className="px-6 py-4 bg-zinc-50 dark:bg-zinc-950/50 transition-colors border-t border-zinc-100 dark:border-zinc-800 transition-colors"><p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">{filtered.length} {filtered.length === 1 ? 'curso' : 'cursos'}</p></div>
            </div>
                    ...
                    <div className="bg-white dark:bg-zinc-900 transition-colors rounded-3xl shadow-2xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleSave} className="p-8 space-y-5">
                            ...
                            <div>
                                ...
                                <textarea value={form.descricao} onChange={e => setForm(f => ({ ...f, descricao: e.target.value }))} className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-950/50 transition-colors border border-zinc-200 dark:border-zinc-700 transition-colors rounded-xl text-sm font-semibold text-[#231F20] dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-[#E31E24] h-24 resize-none" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="number" step="0.01" value={form.valor_rematricula} onChange={e => setForm(f => ({ ...f, valor_rematricula: parseFloat(e.target.value) || 0 }))} className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-950/50 transition-colors border border-zinc-200 dark:border-zinc-700 transition-colors rounded-xl text-sm font-semibold text-[#231F20] dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-[#E31E24]" />
                                </div>
                            </div>
