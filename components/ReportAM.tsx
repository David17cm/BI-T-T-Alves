...
const ReportAM: React.FC = () => {
    ...
    const filteredEnrollments = useMemo(() => {
        return enrollments.filter(e => {
            ...
            return d.getFullYear() === TARGET_YEAR;
        });
    }, [enrollments]);
    ...
    const weeklyEvolution = useMemo(() => {
        // Fixed range for 2026
        const startDate = new Date(TARGET_YEAR, 0, 1); // Jan 1st

        // Define lastDate: if current year is TARGET_YEAR, use today. Else use Dec 31st.
        const today = new Date();
        let lastDate = new Date(TARGET_YEAR, 11, 31);

        if (today.getFullYear() === TARGET_YEAR) {
            lastDate = today;
        }

        // Normalize start date to beginning of that day
        startDate.setHours(0, 0, 0, 0);
        // Normalize last date to end of that day
        ...
        while (currentStart <= lastDate && safetyCounter < MAX_WEEKS) {
            ...
            weeks.push({
                week: labelDate,
                fullLabel: `${labelDate} à ${labelEnd}`,
                count
            });
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-zinc-500 text-[9px] font-bold uppercase tracking-widest mt-1">
                                {filteredEnrollments.length} matrículas em 2026
                            </p>
                        </div>
                        <div className="text-right">
                            <p className="text-[#FFF200] text-3xl font-black italic">{filteredEnrollments.length}</p>
                            <p className="text-zinc-500 text-[8px] font-bold uppercase">matrículas 2026</p>
                        </div>
                    </div>
                </div>
