    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <h2 className="text-4xl font-black text-[#231F20] dark:text-zinc-100 uppercase tracking-tighter italic leading-none">Gestão de <span className="text-[#E31E24]">Alunos</span></h2>
                ...
                {loading ? <div className="flex items-center justify-center py-16"><div className="w-10 h-10 border-4 border-zinc-200 dark:border-zinc-700 transition-colors border-t-[#E31E24] rounded-full animate-spin"></div></div> : (
                    <table className="w-full text-left">
                        ...
                        <tbody>
                            ...
                            {filtered.map((a, i) => (
                                <tr key={a.id} className={`border-t border-zinc-100 dark:border-zinc-800 transition-colors hover:bg-zinc-50 dark:bg-zinc-950/50 transition-colors ${i % 2 ? 'bg-zinc-50 dark:bg-zinc-950/50 /50 transition-colors' : ''}`}>
                                    ...
                                    <td className="px-6 py-4"><div className="flex items-center justify-center gap-2">
                                        <button onClick={() => openEdit(a)} className="p-2 bg-zinc-100 dark:bg-zinc-800 transition-colors rounded-lg hover:bg-[#E31E24] hover:text-white transition-all text-zinc-500">{editIcon}</button>
                                        {deleteConfirm === a.id ? (<div className="flex gap-1">
                                            ...
                                            <button onClick={() => setDeleteConfirm(a.id)} className="p-2 bg-zinc-100 dark:bg-zinc-800 transition-colors rounded-lg hover:bg-red-600 hover:text-white transition-all text-zinc-500">{deleteIcon}</button>
                                        )}
                                    </div></td>
                                </tr>))}
                        </tbody>
                    </table>)}
                <div className="px-6 py-4 bg-zinc-50 dark:bg-zinc-950/50 transition-colors border-t border-zinc-100 dark:border-zinc-800 transition-colors"><p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">{filtered.length} aluno(s)</p></div>
            </div>
            {modalOpen && (
                        ...
                        <form onSubmit={handleSave} className="p-8 space-y-5">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div><label className="block text-[9px] font-black text-zinc-500 uppercase tracking-widest mb-1.5">CPF</label>
                                    <input type="text" value={form.cpf} onChange={e => setForm(f => ({ ...f, cpf: e.target.value }))} className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-950/50 transition-colors border border-zinc-200 dark:border-zinc-700 transition-colors rounded-xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-[#E31E24]" /></div>
                            </div>
                            <div><label className="block text-[9px] font-black text-zinc-500 uppercase tracking-widest mb-1.5">Endereço</label>
                                <input type="text" value={form.endereco} onChange={e => setForm(f => ({ ...f, endereco: e.target.value }))} className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-950/50 transition-colors border border-zinc-200 dark:border-zinc-700 transition-colors rounded-xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-[#E31E24]" /></div>
                            <div><label className="block text-[9px] font-black text-zinc-500 uppercase tracking-widest mb-1.5">Vendedor</label>
                                <select value={form.vendedor_id ?? ''} onChange={e => setForm(f => ({ ...f, vendedor_id: e.target.value ? Number(e.target.value) : null }))} className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-950/50 transition-colors border border-zinc-200 dark:border-zinc-700 transition-colors rounded-xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-[#E31E24]">
