    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <h2 className="text-4xl font-black text-[#231F20] dark:text-zinc-100 uppercase tracking-tighter italic leading-none">Gestão de <span className="text-[#E31E24]">Vendedores</span></h2>
                <div className="flex items-center gap-3">
                    <div className="relative">
                        ...
                        <input type="text" placeholder="Buscar..." value={search} onChange={e => setSearch(e.target.value)} className="pl-10 pr-4 py-3 bg-white dark:bg-zinc-900 transition-colors border border-zinc-200 dark:border-zinc-700 transition-colors rounded-xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-[#E31E24] w-64" />
                    </div>
            ...
            <div className="bg-white dark:bg-zinc-900 transition-colors rounded-[2rem] shadow-sm overflow-hidden">
                {loading ? <div className="flex items-center justify-center py-16"><div className="w-10 h-10 border-4 border-zinc-200 dark:border-zinc-700 transition-colors border-t-[#E31E24] rounded-full animate-spin"></div></div> : (
                    <table className="w-full text-left">
                        ...
                        <tbody>
                            ...
                            {filtered.map((v, i) => (
                                <tr key={v.id} className={`border-t border-zinc-100 dark:border-zinc-800 transition-colors hover:bg-zinc-50 dark:bg-zinc-950/50 transition-colors ${i % 2 ? 'bg-zinc-50 dark:bg-zinc-950/50 /50 transition-colors' : ''}`}>
                                    <td className="px-6 py-4">
                                        {v.foto_url ? (
                                            <img src={v.foto_url} alt={v.nome} className="w-10 h-10 rounded-full object-cover shadow-sm border-2 border-zinc-200 dark:border-zinc-700 transition-colors" />
                                        ) : (
                                    ...
                                    <td className="px-6 py-4"><div className="flex items-center justify-center gap-2">
                                        <button onClick={() => openEdit(v)} className="p-2 bg-zinc-100 dark:bg-zinc-800 transition-colors rounded-lg hover:bg-[#E31E24] hover:text-white transition-all text-zinc-500">{editIcon}</button>
                                        {deleteConfirm === v.id ? (<div className="flex gap-1">
                                            ...
                                            <button onClick={() => setDeleteConfirm(v.id)} className="p-2 bg-zinc-100 dark:bg-zinc-800 transition-colors rounded-lg hover:bg-red-600 hover:text-white transition-all text-zinc-500">{deleteIcon}</button>
                                        )}
                                    </div></td>
                                </tr>))}
                        </tbody>
                    </table>)}
                <div className="px-6 py-4 bg-zinc-50 dark:bg-zinc-950/50 transition-colors border-t border-zinc-100 dark:border-zinc-800 transition-colors"><p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">{filtered.length} vendedor(es)</p></div>
            </div>
            {modalOpen && (
                        ...
                        <form onSubmit={handleSave} className="p-8 space-y-5">
                            ...
                            <div className="flex items-center gap-6">
                                <div className="relative group">
                                    {form.foto_url ? (
                                        <img src={form.foto_url} alt="Foto" className="w-20 h-20 rounded-full object-cover shadow-lg border-3 border-zinc-200 dark:border-zinc-700 transition-colors" />
                                    ) : (
                                        <div className="w-20 h-20 rounded-full bg-zinc-100 dark:bg-zinc-800 transition-colors flex items-center justify-center border-2 border-dashed border-zinc-300">
                                ...
                                <div>
                                    ...
                                    <button type="button" onClick={() => fileInputRef.current?.click()} className="px-4 py-2 bg-zinc-100 dark:bg-zinc-800 text-zinc-700 font-bold text-xs rounded-lg hover:bg-zinc-200 transition-all">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="block text-[9px] font-black text-zinc-500 uppercase tracking-widest mb-1.5">Telefone</label>
                                    <input type="text" value={form.telefone} onChange={e => setForm(f => ({ ...f, telefone: e.target.value }))} className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-950/50 transition-colors border border-zinc-200 dark:border-zinc-700 transition-colors rounded-xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-[#E31E24]" /></div>
                            ...
                            <div><label className="block text-[9px] font-black text-zinc-500 uppercase tracking-widest mb-1.5">Comissão (%)</label>
                                <input type="number" step="0.1" value={form.comissao} onChange={e => setForm(f => ({ ...f, comissao: parseFloat(e.target.value) || 0 }))} className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-950/50 transition-colors border border-zinc-200 dark:border-zinc-700 transition-colors rounded-xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-[#E31E24]" /></div>
