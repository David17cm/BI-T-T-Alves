...
const App: React.FC = () => {
  ...
  useEffect(() => {
    getSession().then(session => {
      ...
      setRole(r);
      if (r === 'auxiliar') {
        setActiveTab(prev => prev === 'overview' ? 'enrollments' : prev);
      } else if (r === 'vendedor') {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      let page = 0;
      let allRows: EnrollmentDataWithId[] = [];
      let hasMore = true;

      while (hasMore) {
        const rows = await fetchEnrollments({ page, pageSize: PAGE_SIZE });
  ...
  const filteredDailyData = useMemo(() => {
    ...
    // Sempre usa hoje como data de referência
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen bg-[#F8F9FA] flex items-center justify-center">
        <div className="w-12 h-12 border-4 border-zinc-200 dark:border-zinc-700 transition-colors border-t-[#E31E24] rounded-full animate-spin" />
      </div>
    );
  }

  // Login gate removido pois agora o Router gerencia a proteção pela rota.
  // if (!user) {
  ...
  return (
    <Routes>
      ...
      <div className="md:hidden bg-[#231F20] text-white p-4 flex items-center justify-between sticky top-0 z-30 shadow-md">
        ...
        <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 text-white hover:bg-white dark:bg-zinc-900 transition-colors/10 rounded-lg">
      ...
      <aside className={`fixed inset-y-0 left-0 bg-[#231F20] text-white flex flex-col shadow-2xl z-40 transition-transform duration-300 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:sticky md:top-0 md:h-screen w-72 md:w-72`}>
        <div className="p-8 border-b border-white/10 flex flex-col items-center relative">
          ...
          <div className="mt-4 flex flex-col items-center gap-1 w-full">
            <span className={`text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest ${isAdmin ? 'bg-[#E31E24] text-white' : isVendedor ? 'bg-amber-500 text-white' : 'bg-white dark:bg-zinc-900 transition-colors/10 text-zinc-400'}`}>
              {isAdmin ? '👑 Admin' : isVendedor ? '📞 Vendedor' : '👤 Auxiliar'}
            </span>
        ...
        <nav className="flex-grow p-4 space-y-2 overflow-y-auto custom-scrollbar">
          ...
          {isAdmin && (
            <>
              <button onClick={() => setActiveTab('overview')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'overview' ? 'bg-[#E31E24] text-white shadow-lg' : 'text-zinc-400 hover:bg-white dark:bg-zinc-900 transition-colors/5 hover:text-white'}`}>
              ...
              <button onClick={() => setActiveTab('commercial')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'commercial' ? 'bg-[#E31E24] text-white shadow-lg' : 'text-zinc-400 hover:bg-white dark:bg-zinc-900 transition-colors/5 hover:text-white'}`}>
          ...
          {/* Menus Ocultos para Vendedores */}
          {!isVendedor && (
            <>
              <button onClick={() => setActiveTab('enrollments')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'enrollments' ? 'bg-[#E31E24] text-white shadow-lg' : 'text-zinc-400 hover:bg-white dark:bg-zinc-900 transition-colors/5 hover:text-white'}`}>
              ...
              <button onClick={() => setActiveTab('vendedores')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'vendedores' ? 'bg-[#E31E24] text-white shadow-lg' : 'text-zinc-400 hover:bg-white dark:bg-zinc-900 transition-colors/5 hover:text-white'}`}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                Vendedores
              </button>
              <button onClick={() => setActiveTab('trafego')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'trafego' ? 'bg-[#E31E24] text-white shadow-lg' : 'text-zinc-400 hover:bg-white dark:bg-zinc-900 transition-colors/5 hover:text-white'}`}>
          ...
          {!isVendedor && (
            <button onClick={() => setActiveTab('cobranca')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'cobranca' ? 'bg-[#E31E24] text-white shadow-lg' : 'text-zinc-400 hover:bg-white dark:bg-zinc-900 transition-colors/5 hover:text-white'}`}>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
              Cobrança
            </button>
          )}
          {!isVendedor && (
            <button onClick={() => setActiveTab('desempenho')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'desempenho' ? 'bg-[#E31E24] text-white shadow-lg' : 'text-zinc-400 hover:bg-white dark:bg-zinc-900 transition-colors/5 hover:text-white'}`}>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
              Desempenho
            </button>
          )}
          {isAdmin && (
            <button onClick={() => setActiveTab('report_am')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'report_am' ? 'bg-[#E31E24] text-white shadow-lg' : 'text-zinc-400 hover:bg-white dark:bg-zinc-900 transition-colors/5 hover:text-white'}`}>
          ...
          {isMaster && (
            <>
              ...
              <button onClick={() => setActiveTab('usuarios')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'usuarios' ? 'bg-[#E31E24] text-white shadow-lg' : 'text-zinc-400 hover:bg-white dark:bg-zinc-900 transition-colors/5 hover:text-white'}`}>
              ...
              <button onClick={() => setActiveTab('auditoria')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'auditoria' ? 'bg-[#E31E24] text-white shadow-lg' : 'text-zinc-400 hover:bg-white dark:bg-zinc-900 transition-colors/5 hover:text-white'}`}>
          ...
          {/* Negociações: Master ou Vendedores (Produção) */}
          {(isMaster || isVendedor) && (
            <>
              {!isVendedor && import.meta.env.DEV === false && (
                <div className="mt-4 pt-4 border-t border-white/10">
                  <p className="text-[9px] font-black text-amber-500 uppercase tracking-widest mb-2 px-4">Operacional</p>
                </div>
              )}
              <button onClick={() => setActiveTab('negociacoes')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'negociacoes' ? 'bg-[#E31E24] text-white shadow-lg' : isVendedor ? 'text-zinc-400 hover:bg-white dark:bg-zinc-900 transition-colors/5 hover:text-white' : 'text-amber-400/70 hover:bg-amber-500/10 hover:text-amber-400'}`}>
        ...
        <div className="p-4 border-t border-white/10 flex flex-col gap-2">
          ...
          <button
            onClick={toggleTheme}
            className={`w-full flex items-center gap-3 px-4 py-3 font-bold uppercase tracking-widest text-[9px] rounded-xl transition-all duration-300 ${isDark ? 'bg-indigo-900/30 text-indigo-300 hover:bg-indigo-800/50' : 'bg-amber-500/10 text-amber-500 hover:bg-amber-500/20'}`}
          >
            <div className={`w-6 h-6 rounded-lg flex items-center justify-center transition-colors ${isDark ? 'bg-indigo-800/50' : 'bg-amber-500/20'}`}>
              {isDark ? (
          ...
          {isMaster && (
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-3 bg-zinc-800/50 text-zinc-300 font-bold uppercase tracking-widest text-[9px] rounded-xl hover:bg-zinc-800 hover:text-white transition-all group"
            >
      ...
      <main className="flex-1 p-4 md:p-10 overflow-y-auto custom-scrollbar w-full">
        ...
        {loading && (
          <div className="h-full flex flex-col items-center justify-center text-center">
            <div className="w-16 h-16 border-8 border-zinc-200 dark:border-zinc-700 transition-colors border-t-[#E31E24] rounded-full animate-spin mb-6 mx-auto"></div>
        ...
        {/* ERROR STATE */}
        {error && !loading && (
          <div className="h-full flex flex-col items-center justify-center text-center p-10 bg-white dark:bg-zinc-900 rounded-[3rem] shadow-inner border-4 border-dashed border-red-200 dark:border-red-900/40 transition-colors">
        ...
        {/* EMPTY STATE */}
        {!stats && !loading && !error && !isVendedor && (activeTab === 'overview' || activeTab === 'commercial') && (
          <div className="h-full flex flex-col items-center justify-center text-center p-10 bg-white dark:bg-zinc-900 rounded-[3rem] shadow-inner border-4 border-dashed border-zinc-100 dark:border-zinc-800 transition-colors">
            <h1 className="text-3xl font-black text-[#231F20] dark:text-zinc-100 uppercase tracking-tighter italic">BI <span className="text-[#E31E24]">FINANCEIRO</span></h1>
        ...
        {stats && !isVendedor && (activeTab === 'overview' || activeTab === 'commercial') && (
          <div className="max-w-6xl mx-auto space-y-10 animate-in fade-in duration-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {!isVendedor && (isAdmin ? (
                <>
                  <div className="bg-white dark:bg-zinc-900 rounded-[2rem] p-6 shadow-xl border-l-[10px] border-[#E31E24] transition-colors">
                    ...
                    <div className="flex items-end gap-1">
                      ...
                      <h3 className="text-3xl font-black text-[#231F20] dark:text-white tracking-tighter">{stats.totalSales.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
                    </div>
                  </div>
                  <div className="bg-white dark:bg-zinc-900 rounded-[2rem] p-6 shadow-xl border-l-[10px] border-[#FFF200] transition-colors">
                    ...
                    <div className="flex items-end gap-1">
                      ...
                      <h3 className="text-3xl font-black text-[#231F20] dark:text-white tracking-tighter">{stats.totalReceived.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
                    </div>
                  </div>
                  <div className="bg-white dark:bg-zinc-900 rounded-[2rem] p-6 shadow-xl border-l-[10px] border-[#231F20] dark:border-zinc-700 transition-colors">
                    <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-1">Matrículas</p>
                    <div className="flex items-end gap-1">
                      <h3 className="text-3xl font-black text-[#231F20] dark:text-white tracking-tighter">{stats.totalEnrollments}</h3>
                ...
                <div className="md:col-span-4 bg-white dark:bg-zinc-900 transition-colors rounded-[2rem] p-6 shadow-xl border-l-[10px] border-[#231F20] flex items-center gap-4">
                  <div>
                    <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-1">Matrículas</p>
                    <div className="flex items-end gap-1">
                      <h3 className="text-3xl font-black text-[#231F20] dark:text-zinc-100 tracking-tighter">{stats.totalEnrollments}</h3>
                      <span className="text-xs font-black text-zinc-400 mb-1 uppercase italic">Unid.</span>
                    </div>
                  </div>
                </div>
            ...
            {activeTab === 'overview' && !isVendedor && (
              <div className="space-y-10">
                {/* LISTAGEM POR PACOTE */}
                <section className="bg-white dark:bg-zinc-900 p-10 rounded-[3rem] shadow-sm transition-colors">
                  <h4 className="text-xs font-black text-[#231F20] dark:text-zinc-100 uppercase tracking-[0.2em] flex items-center gap-3 mb-10">
                    <span className="w-3 h-6 bg-[#E31E24] rounded-full"></span> Resultados por Pacote
                  </h4>
                  <div className="grid grid-cols-1 gap-6">
                    {stats.courseMetrics?.map((course, idx) => (
                      <div key={idx} className="bg-zinc-50 dark:bg-zinc-950/50 rounded-[2rem] p-8 border border-zinc-100 dark:border-zinc-800 flex flex-col lg:flex-row lg:items-center gap-8 hover:shadow-md transition-all">
                        <div className="lg:w-1/3">
                          <span className="text-[9px] font-black text-[#E31E24] uppercase tracking-widest block mb-1">Nome do Pacote</span>
                          <h5 className="text-lg font-black text-[#231F20] dark:text-white uppercase tracking-tighter leading-tight">{course.name}</h5>
                          <span className="inline-block mt-2 bg-[#231F20] dark:bg-zinc-800 text-white text-[9px] font-black px-3 py-1 rounded-full">{course.enrollmentCount} VENDIDOS</span>
                        </div>
                        <div className="flex-grow grid grid-cols-2 gap-4 border-l border-zinc-200 dark:border-zinc-800 lg:pl-10">
                          <div>
                            <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-1">Total Gerado</p>
                            <p className="text-xl font-black text-[#231F20] dark:text-white">R$ {course.totalSales.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                          </div>
                          <div>
                            <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-1">Total Recebido</p>
                            <p className="text-xl font-black text-green-600 dark:text-green-500">R$ {course.totalReceived.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                          </div>
                        </div>
                        <div className="lg:w-48 text-right bg-white dark:bg-zinc-900 p-6 rounded-3xl shadow-inner border-t-8 border-[#E31E24]">
                          <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-1">Efetividade</p>
                          <p className="text-3xl font-black text-[#231F20] dark:text-white tracking-tighter">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  <div className="lg:col-span-2 bg-white dark:bg-zinc-900 p-10 rounded-[3rem] shadow-sm transition-colors">
                    <div className="flex flex-col gap-4 mb-8">
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <h4 className="text-[10px] font-black text-[#231F20] dark:text-zinc-100 uppercase tracking-[0.2em]">Fluxo de Matrículas</h4>
                        <div className="flex flex-wrap gap-2">
                          {[{ id: 'tudo', label: 'TUDO' }, { id: 'ontem', label: 'ONTEM' }, { id: '7dias', label: '7 DIAS' }, { id: '14dias', label: '14 DIAS' }, { id: '30dias', label: '30 DIAS' }, { id: 'personalizado', label: 'PERSONALIZADO' }].map((f) => (
                            <button key={f.id} onClick={() => setDateFilter(f.id as DateFilter)} className={`px-3 py-1.5 text-[8px] font-black uppercase tracking-widest rounded-lg transition-all ${dateFilter === f.id ? 'bg-[#E31E24] text-white shadow-lg' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:bg-zinc-200'}`}>{f.label}</button>
                          ))}
                        </div>
                      </div>
                      {dateFilter === 'personalizado' && (
                        <div className="flex flex-wrap items-center gap-3 bg-zinc-50 dark:bg-zinc-950/50 transition-colors rounded-2xl px-5 py-3">
                          <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">De:</span>
                          <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="bg-white dark:bg-zinc-900 transition-colors border border-zinc-200 dark:border-zinc-700 transition-colors rounded-lg px-3 py-1.5 text-xs font-bold text-[#231F20] dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-[#E31E24]/30 focus:border-[#E31E24]" />
                          <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">Até:</span>
                          <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="bg-white dark:bg-zinc-900 transition-colors border border-zinc-200 dark:border-zinc-700 transition-colors rounded-lg px-3 py-1.5 text-xs font-bold text-[#231F20] dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-[#E31E24]/30 focus:border-[#E31E24]" />
                          {startDate && endDate && (
                      ...
                      <div className="flex items-center gap-6">
                        <span className="text-[10px] font-bold text-zinc-400">
                          <span className="text-[#231F20] dark:text-zinc-100 font-black">{filteredDailyData.reduce((s, d) => s + d.count, 0)}</span> matrícula{filteredDailyData.reduce((s, d) => s + d.count, 0) !== 1 ? 's' : ''} no período
                        </span>
                  ...
                  <div className="bg-white dark:bg-zinc-900 p-10 rounded-[3rem] shadow-sm transition-colors">
                    <h4 className="text-[10px] font-black text-[#231F20] dark:text-zinc-100 uppercase tracking-[0.2em] mb-8">Status das Vendas</h4>
              ...
              <div className="bg-[#231F20] p-10 rounded-[3rem] shadow-2xl">
                ...
                <div className="grid grid-cols-1 gap-4">
                  {stats.attendantMetrics?.map((attendant, index) => (
                    <div key={index} className="bg-white dark:bg-zinc-900 transition-colors/5 p-8 rounded-[2rem] border border-white/10 flex flex-col lg:flex-row lg:items-center gap-6 group hover:bg-white dark:bg-zinc-900 transition-colors/10 transition-all">
